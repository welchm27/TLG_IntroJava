...
public class HourlyEmployee extends Employee {
    ...
    public void setRate(double rate) throws IllegalWageException {
        if (FEDERAL_MINIMUM_WAGE <= rate) {
            this.rate = rate;
        } else {
            throw new IllegalWageException(String.format("%s's rate is less than the Federal Minimum Wage of %s",
                    getName(), FEDERAL_MINIMUM_WAGE));
        }
    }
